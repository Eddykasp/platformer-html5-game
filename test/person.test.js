...
test('person constructor', t => {
    ...
    t.equal(p.pos.x, 0, 'x position');
    t.equal(p.pos.y, 0, 'y position');
...
test('person move positive x / positive y', t => {
    var p = new Person(0, 0);
    t.equal(p.pos.x, 0, 'x position before moving');
    t.equal(p.pos.y, 0, 'y position before moving');
    p.xv = 10;
    p.yv = 10;
    p.move();
    t.equal(p.pos.x, 10, 'x position after moving');
    t.equal(p.pos.y, 10, 'y position after moving');
    t.end();
});
